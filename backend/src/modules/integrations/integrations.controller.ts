...
export async function tiktokConnectController(req: Request, res: Response, next: NextFunction) {
  try {
    const result = await startTikTokConnect(req.user!.id);
    if (req.headers.accept?.includes("application/json") || req.query.json === "1") {
      return sendSuccess(res, result);
    }
    return res.redirect(result.authorizationUrl);
...
export async function tiktokCallbackController(req: Request, res: Response, next: NextFunction) {
  const wantsJson = req.headers.accept?.includes("application/json") || req.query.json === "1";
  try {
    const code = typeof req.query.code === "string" ? req.query.code : undefined;
    const state = typeof req.query.state === "string" ? req.query.state : undefined;
